...
void accumulate_metrics(struct per_cpu_event *cpu, int num_events, int mon_type) {
	vector<long long> total_per_event(num_events);
	if (mon_type == mon_mpki) {
		mpki.clear();
	}
	else {
		avgs.clear();
	...
	if (mon_type == mon_mpki) {
		...
		long l1d_miss = total_per_event[3];
		long l1i_miss = total_per_event[4];
		long dtlb_miss = total_per_event[5];
		long itlb_miss = total_per_event[6];
		long l2_miss = total_per_event[7];
		...
		long cpu_clock = total_per_event[0];
		long page_fault = total_per_event[1];
		long cpu_migration = total_per_event[2];
		long context_switch = total_per_event[3];
		long l1d_pending = total_per_event[4];	// for MLP
		long l1d_pending_cycles = total_per_event[5];	// for MLP
...
int perf_count_multicore(int interval, int mon_type) {
	struct perf_event_attr mpki_pe_def[] = {
	...
	struct perf_event_attr avg_pe_def[] = {
	...
	if (mon_type == mon_mpki) {
		ptr = mpki_pe_def;
		num_events = sizeof(mpki_pe_def) / sizeof(struct perf_event_attr);
		printf("mpki num_events: %d\n", num_events);
	}
	else {
		ptr = avg_pe_def;
		num_events = sizeof(avg_pe_def) / sizeof(struct perf_event_attr);
		printf("avg num_events: %d\n", num_events);
	}
	...
	for (int i = 0; i < num_cores; i++) {
		for (int j = 0; j < num_events; j++) {
			pe.type = ptr[j].type;
			pe.config = ptr[j].config;
			pe.exclude_kernel = ptr[j].exclude_kernel;
	...
	while(!stop) {
		...
		accumulate_metrics(cpu, num_events, mon_type);
		// printf("msg: %s\n", metric_msg.c_str());
	}
...
int main(int argc, char *argv[]) {
	...
	thread mpki_perf(perf_count_multicore, mon_interval, mon_mpki);

	this_thread::sleep_for(chrono::milliseconds(mon_interval / 2));	

	thread avg_perf(perf_count_multicore, mon_interval, mon_avg);
