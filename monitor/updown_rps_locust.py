...
class Wave(LoadTestShape):
	time_limit = 1200
	peak_time = time_limit / 2
	user_pad = 5.0
	peak_users = user_pad + 25.0 # constant: max rps
	min_users = 1.0	# constant: min rps
	user_diff = peak_users - min_users
	spawn_rate = round(user_diff / peak_time, 3)

	def tick(self):
		runtime = self.get_run_time()

		if runtime <= self.peak_time:
			users = self.min_users + round(runtime * self.spawn_rate)
			return (users, users)
		elif runtime > self.peak_time and runtime < self.time_limit:
			users = self.peak_users - round((runtime - self.peak_time) * self.spawn_rate)
			return (users, users)
		else:
			None